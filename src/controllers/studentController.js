...
exports.createStudent = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      ...
      (student_id, full_name, email, phone, department, year, qr_code, qr_code_data, enrolled) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [student_id, full_name, email, phone, department, year, qrCodeImage, qrData, true]
    );
...
exports.updateStudent = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE students 
      SET full_name = ?, email = ?, phone = ?, department = ?, year = ?, status = ?
      WHERE id = ?`,
      [
...
exports.getStatistics = async (req, res) => {
  try {
    ...
    const [enrolledStudents] = await db.query(
      'SELECT COUNT(*) as enrolled FROM students WHERE enrolled = true'
    );
    ...
    res.json({
      ...
      data: {
        ...
        enrolledBiometric: enrolledStudents[0].enrolled,
